...
const AppContext = ({ children }) => {
    const [appConfig, setAppConfig] = useState({
        ...
        GOOGLE_CLIENT_ID:
            "857590091173-lm2tl3nqfvp2thd4nrhqidjuq2hroco8.apps.googleusercontent.com",
        ...
        const fetchEntityDetails = async (paramEntityId) => {
            try {
                let apiKeyRequest = generalFunction.createUrl(
                    `api/entities/${generalFunction.getDataFromCookies(
                        "adminCommunityId"
                    )}/keys?userId=${generalFunction.getDataFromCookies(
                        "questUserId"
                    )}`
                );
